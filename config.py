...
# 离线聚类精修（第二阶段去重）
# 说明：在线去重负责实时唯一化，离线聚类在整段视频处理结束后再做一次全局合并，
# 可以修复单帧抖动、短时遮挡导致的漏合并。
ENABLE_CLUSTER_REFINEMENT = True
CLUSTER_THRESHOLD_COSINE = 0.45
CLUSTER_THRESHOLD_EUCLIDEAN = 1.05
CLUSTER_MIN_SIZE = 2
CLUSTERED_RECORD_FILE = 'face_records_frontal_clustered.txt'
CLUSTER_MAPPING_FILE = 'persona_cluster_mapping.csv'
CLUSTER_BEST_FACE_DIR = 'cluster_best_faces'
CLUSTER_BEST_FACE_INDEX_FILE = 'cluster_best_faces.csv'

# 跨视频全局聚类精修（第三阶段）
# 说明：在所有视频处理完成后，按输出根目录汇总所有persona做一次全局聚类，
# 用于统一不同视频中的同一人ID。
ENABLE_GLOBAL_CLUSTER_REFINEMENT = True
GLOBAL_CLUSTER_THRESHOLD_COSINE = 0.45
GLOBAL_CLUSTER_THRESHOLD_EUCLIDEAN = 1.05
GLOBAL_CLUSTER_MIN_SIZE = 2

# 全局匹配总开关
# 说明：控制所有跨运行/跨视频的匹配行为，包括：
#   1. 在线匹配：启动时加载历史 embedding 库 (--reuse-embedding-db)
#   2. 全局聚类：所有视频处理完后做跨视频聚类 (--global-cluster-refine)
# 设置为 False 等价于同时 --no-reuse-embedding-db 和 --no-global-cluster-refine
ENABLE_GLOBAL_MATCH = True
GLOBAL_CLUSTER_MAPPING_FILE = 'global_persona_cluster_mapping.csv'
PER_VIDEO_GLOBAL_CLUSTERED_RECORD_FILE = 'face_records_frontal_global_clustered.txt'
GLOBAL_CLUSTERED_RECORD_FILE = 'face_records_global_clustered.txt'
GLOBAL_CLUSTER_BEST_FACE_DIR = 'global_cluster_best_faces'
GLOBAL_CLUSTER_BEST_FACE_INDEX_FILE = 'global_cluster_best_faces.csv'

# 半身上下文匹配（衣物辅助）
# 说明：ArcFace仍只做人脸embedding，额外引入半身区域颜色直方图做辅助匹配，
# 以提升同视频/近时间段内的稳定性（尤其在人脸角度变化或轻微遮挡时）。
EMBEDDING_MODE = 'half_body'  # 'half_body' | 'face'

# ReID 深度特征（行人重识别）
# 说明：使用 OSNet-x0.25 在 MSMT17 数据集上预训练的模型提取半身图片的 512 维 embedding，
# 比 HSV 直方图有更强的语义区分能力（颜色+纹理+人体结构），换衣服也能匹配。
# 需要先运行 setup_project.py 下载模型到 models/reid/
REID_MODEL_NAME = 'osnet_x0_25_msmt17'
REID_MODEL_PATH = 'models/reid/osnet_x0_25_msmt17.pt'  # 相对路径
REID_INPUT_SIZE = (256, 128)  # (H, W) 标准行人图尺寸
REID_FEATURE_DIM = 512
USE_REID_FEATURE = True  # True=使用 ReID 深度特征; False=使用 HSV 直方图

# 聚类精修特征模式（决定聚类阶段使用哪种特征提取模型）
# 'reid'   : OSNet-x0.25 行人 ReID（看半身衣着，适合区分穿搭不同的人）
# 'arcface': InsightFace w600k_r50 ArcFace（看人脸五官，适合精确判断是否同一人，推荐）
# 'hsv'    : HSV 颜色直方图（传统方法，不依赖模型）
CLUSTER_FEATURE_MODE = 'reid'  # 默认值，可通过 --cluster-feature 参数覆盖

# ArcFace 人脸特征（聚类精修专用）
# 说明：使用 InsightFace 的 w600k_r50 模型，通过人脸五官判断是否同一人。
# 比 OSNet ReID 区分度高很多（同一人 0.85~0.95+，不同人 0.30~0.55）。
# 需要 InsightFace 已安装，且 buffalo_l 模型已下载。
ARCFACE_MODEL_ROOT = 'models/insightface'  # InsightFace 模型根目录（包含 buffalo_l/）
ARCFACE_MODEL_NAME = 'buffalo_l'  # InsightFace 模型名称
ARCFACE_FEATURE_DIM = 512
ARCFACE_DET_SIZE = (640, 640)  # 人脸检测尺寸

USE_CONTEXT_REID = True
CONTEXT_FACE_WEIGHT = 0.8
CONTEXT_APPAREL_WEIGHT = 0.2
CONTEXT_HIST_BINS = 8

# 半身截图参数（基于人脸框扩展）
SAVE_HALF_BODY_IMAGE = True
HALF_BODY_TOP_EXPAND = 0.25
HALF_BODY_BOTTOM_EXPAND = 2.2
HALF_BODY_SIDE_EXPAND = 0.8
