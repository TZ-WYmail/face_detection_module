...
def download_via_direct_url(target_dir: Path) -> bool:
    ...
    file_map = {
        ...
        "2d106det.onnx":   "landmark/2d106det.onnx",
        "1k3d68.onnx":     "landmark/1k3d68.onnx",
...
""",
    ...
    parser.add_argument(
        ...
        help="模型保存目录 (默认: ~/.insightface/models/buffalo_l)",
    )
    parser.add_argument(
        "--method",
    ...
    if args.insightface_dir:
        ...
        target_dir = Path.home() / ".insightface" / "models" / "buffalo_l"
    target_dir.mkdir(parents=True, exist_ok=True)
    ...
    if all(results.values()):
        ...
        logger.info("  app = FaceAnalysis(name='buffalo_l', root='~/.insightface/models')")
...
echo "✅ download_insightface.py 创建完成"